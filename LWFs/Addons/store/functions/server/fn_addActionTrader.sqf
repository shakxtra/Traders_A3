/*
	Function: LWFs_store_fnc_addActionTrader
	Author: LWFs
	...
	Example:
	[trader1, west] call LWFs_store_fnc_addActionTrader;
...
if (_target isEqualType false) then {
	if (hasInterface) then {
		[
			_trader,
			format [localize "STR_LWFs_STORE_OPEN_ACTION", _trader getVariable ["LWFs_store_name", localize "STR_LWFs_STORE_ACTION"]],
			...
			{},
			{
				...
				[_trader] call LWFs_store_fnc_openStore;
			}, {}, [], 0.5, nil, false, false
		] call BIS_fnc_holdActionAdd;
	};
} else {
	if (isServer) then {
		[_trader] remoteExecCall ["LWFs_store_fnc_addActionTrader", _target, _trader];
	};
};