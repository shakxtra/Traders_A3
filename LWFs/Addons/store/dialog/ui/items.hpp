...
class ItemCheckboxSort: RscItemCheckboxGreen {
	...
	w = GRID_X(BAR_W);
	h = GRID_Y(BAR_H);
	onCheckedChanged = "['listbox', ['sort', _this]] call LWFs_store_fnc_main;";
	sizeEx = FONT(1);
	color[] = {1, 1, 1, 0.7};
	colorActive[] = {1, 1, 1, 1};
...
class ItemCheckboxAvailable: RscItemCheckboxGreen {
	...
	w = GRID_X(BAR_W);
	h = GRID_Y(BAR_H);
	onCheckedChanged = "['listbox', ['update', []]] call LWFs_store_fnc_main;";
	tooltip = "$STR_LWFs_STORE_CHECKBOX_AVALIABLE";
};
...
class ItemCheckboxCompatible: RscItemCheckboxGreen {
	...
	h = GRID_Y(BAR_H);
	onCheckedChanged = "['listbox', ['update', []]] call LWFs_store_fnc_main;";
	tooltip = "$STR_LWFs_STORE_CHECKBOX_COMPATIBLE";
};
...
class ItemCheckboxSell: RscItemCheckboxGreen {
	...
	w = GRID_X(BAR_W);
	h = GRID_Y(BAR_H);
	onCheckedChanged = "['button', ['change', _this]] call LWFs_store_fnc_main; ['listbox', ['update', []]] call LWFs_store_fnc_main;";
	tooltip = "$STR_LWFs_STORE_CHECKBOX_SELLFILTER";
};
